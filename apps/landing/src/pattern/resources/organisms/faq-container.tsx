    ...
    return (
        <div className='w-full h-fit lg:h-[625px] flex items-center justify-center'>
            <div className='w-full h-full flex flex-col items-start gap-y-3'>
                <h3 className='w-full font-rubik font-semibold text-foreground text-[2rem] text-left pb-6 lg:pb-12 border-b'>{header}</h3>
                    ...
                    <Accordion type='single' collapsible className='w-full'>
                        {faqs?.map(({ question, answer }, idx) => (
                            <AccordionItem key={idx} value={`${idx}`} className='space-y-[12px]'>
                                ...
                                <AccordionContent className='text-base text-accent-foreground text-left max-w-[1100px]'>{answer}</AccordionContent>
                            </AccordionItem>
                        ))}
                    </Accordion>
                </div>
