...
const HowItWorksStep: FC<IProps> = ({ description, number, title, value, toggleGroupValue }) => {
  return (
    <ToggleGroupItem value={value} aria-label={title} size="custom" className='h-full focus-visible:bg-transparent hover:bg-transparent data-[state=on]:bg-transparent transition-all duration-200 ease-in-out'>
      <div className='w-full h-full flex items-start gap-6 lg:gap-8'>
        ...
        <div className='w-full max-w-[271px] lg:max-w-fit lg:min-w-[363px] lg:w-fit flex flex-col items-start gap-y-[18px] mt-[13px]'>
          {/* Title */}
          <h4 className={cn(toggleGroupValue === value ? 'text-foreground' : 'text-muted-foreground', 'text-left text-xl lg:text-2xl whitespace-pre-wrap lg:whitespace-nowrap font-dmsans font-bold transition-colors duration-200 ease-in-out')}>{title}</h4>

          {/* Description */}
          <Hidden isVisible={toggleGroupValue === value} >
            <p className='max-w-[271px] lg:max-w-fit text-left text-sm lg:text-base text-gray-600 font-dmsans transition-colors duration-200 ease-in-out'>{description}</p>
          </Hidden>
        </div>
      </div>
    </ToggleGroupItem>
